...
from attention.test.attention_predict_beam import attention_predict_beam
from attention.test.predict_greedy import predict_greedy
...
def multi_acc_compute(net, preprocess_info_dict, src_vocab, tgt_vocab, num_steps, device, seed):
    ...
    for i in range(len(test_eng_list)):
        ...
        if predictor == 'greedy':
            ...
            pred_id_list, pred, criterion, attention_weight = \
                attention_predict_beam(net, src_sentence, src_vocab, tgt_vocab, num_steps, device)
    ...
    info_dict = torch.load(paths.attention_record_path + str(seed) + '/info_dict')
...
def extrapolate_acc_compute(net, preprocess_info_dict, src_vocab, tgt_vocab, num_steps, device, seed):
    ...
    for i in range(len(test_eng_list)):
        ...
        if predictor == 'greedy':
            ...
            pred_id_list, pred, criterion, attention_weight = \
                attention_predict_beam(net, src_sentence_split, src_vocab, tgt_vocab, num_steps, device)
        # metric 1: string accuracy
        str_acc = string_accuracy.str_acc_compute(reference_id_list, pred_id_list, tgt_vocab)
...
if __name__ == '__main__':
    for seed in seed_list:
        ...
        if device != torch.device('cpu'):
            net.load_state_dict(torch.load(paths.attention_record_path + str(seed) + '/net_state_dict'))
            net.to(device)
        else:  # cpu is used
            net.load_state_dict(torch.load(paths.attention_record_path + str(seed) + '/net_state_dict',

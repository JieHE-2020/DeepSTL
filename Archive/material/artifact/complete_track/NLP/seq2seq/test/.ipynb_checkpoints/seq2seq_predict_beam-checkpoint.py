...
class Candidate:
    ...
    def dec_state_process(self, dec_state):
        self.dec_state = dec_state.clone().to(seq2seq_hyperparas.device)
    
    def update(self):
        self.last_element_tensor = torch.tensor([[self.seq_list[-1]]], device=seq2seq_hyperparas.device)
        ...
        self.criterion = self.log_sum / (self.seq_len ** seq2seq_hyperparas.alpha)


def seq2seq_predict_beam(net, src_sentence, src_vocab, tgt_vocab, num_steps, device, save_seq2seq_weights=False):
    net.eval()
    ...
    # prepare 2 lists
    output_id_list, seq2seq_weight_seq = [], []
    ...
    while step < num_steps and len(complete_sent_list) < seq2seq_hyperparas.topk:
        """
        seq2seq:
        ...
        if step == 0:
            ...
            topk_value_list = heapq.nlargest(seq2seq_hyperparas.enlarge_factor * seq2seq_hyperparas.topk, Y1)
            ...
            for candidate in selected_list:
                ...
                Y1 = Y1[0].cpu().detach().numpy().tolist()
                topk_value_list = \
                    heapq.nlargest(seq2seq_hyperparas.enlarge_factor * seq2seq_hyperparas.topk, Y1)
                topk_index_list = list(map(Y1.index, topk_value_list))
                # print(topk_value_list)
                # print(topk_index_list)
        ...
        for j in range(len(temp_candidate_list)):
            ...
            if len(complete_sent_list) == seq2seq_hyperparas.topk or \
                    len(selected_list) == seq2seq_hyperparas.topk:
                break
        step = step + 1
    ...
    return output_id_list, tgt_vocab.decode(output_id_list), criterion, seq2seq_weight_seq

...
class Seq2seqTrainerValidator:
    ...
    def read_file(self, seed):
        self.checkpoint_dict = torch.load(paths.seq2seq_record_path + str(seed) + '/checkpoint_dict')
        # read model state
        ...
        f = open(paths.seq2seq_record_path + str(seed) + '/data_iter_dict', 'rb')
    ...
    def lrate_compute(self):
        return seq2seq_hyperparas.factor * seq2seq_hyperparas.num_hiddens ** (-0.5) * \
               min(self.step ** (-0.5), self.step * seq2seq_hyperparas.warmup_steps ** (-1.5))
        # use constant learning rate for test
        # return seq2seq_hyperparas.lr
    ...
    def log_write(log, seed):
        with open(paths.seq2seq_record_path + str(seed) + '/log.txt', 'a') as f:
            f.write(log + '\n')
    ...
    def checkpoint_write_read(self, train_loss, train_acc, validate_loss, validate_acc, seed):
        ...
        torch.save(self.checkpoint_dict, paths.seq2seq_record_path + str(seed) + '/checkpoint_dict')
        torch.save(self.checkpoint_dict['net_state_dict'], paths.seq2seq_record_path + str(seed) + '/net_state_dict')
        info_dict = {

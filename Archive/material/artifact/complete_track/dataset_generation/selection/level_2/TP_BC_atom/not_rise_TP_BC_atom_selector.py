from grammar.level_2.TP_atom_BC_atom import not_rise_TP_SP_rules
from selection.level_2.TP_BC_atom.original_TP_BC_atom_selector import OriginalTPBCAtomSelector
import random
import copy


class NotRiseTPBCAtomSelector(OriginalTPBCAtomSelector):
    def __init__(self, tp_type='random'):
        super().__init__(tp_type)
        self.not_rise_tp_info_dict = self.tp_info_dict
    ...
    def tp_select(self):
        ...
        if self.tp_type == 'eventually':
            ...
            category = random.randint(0, len(not_rise_TP_SP_rules.not_rise_TP) - 1)

        # randomly choose the sub_category of the chosen category (in a weighted manner)
        sum_list = []
        sum = 0.0

        for i in range(len(not_rise_TP_SP_rules.not_rise_TP[category])):
            sum = sum + not_rise_TP_SP_rules.not_rise_TP[category][i]['probability']
            sum_list.append(sum)
        ...
        tp_info_dict = copy.deepcopy(not_rise_TP_SP_rules.not_rise_TP[category][sub_category])
...
# for i in range(1000):
#     not_rise_tp_bc_atom_selector = NotRiseTPBCAtomSelector('random')
#     print(not_rise_tp_bc_atom_selector.not_rise_tp_info_dict)
#     print(i)
#

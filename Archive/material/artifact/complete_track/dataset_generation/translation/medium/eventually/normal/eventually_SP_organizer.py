...
class EventuallySPOrganizer:

    def __init__(self, package_list, template, nest_info_dict, tp_sp_type, limit_num):
        self.eng_temporal_phrase = copy.deepcopy(package_list[0])
        # print(len(self.eng_temporal_phrase))
        self.eng_temporal_clause = copy.deepcopy(package_list[1])
        # print(len(self.eng_temporal_clause))
        self.eng_main_sentence = copy.deepcopy(package_list[2])
        # print(len(self.eng_main_sentence))
        ...
        self.translation_list_phrase = self.phrase_directed_translate(tp_sp_type)
    ...
    def phrase_directed_translate(self, tp_sp_type):
        eng_list = []
        ...
        for phrase in self.eng_temporal_phrase:
            for main in self.eng_main_sentence:
                if not self.check_repetition(phrase, adverb_list):  # if phrase does not contain any word in adverb_list
                    ...
                    if not self.nest_info_dict['hasParallelSuccessor']:  # there is no parallel successor
                        ...
                        eng_list.append(eng)
                else:  # phrase contains some word in adverb_list
                    if not self.check_repetition(main, adverb_list):  # if main does not contain any word in adverb_list
                        if tp_sp_type == 'original_TP_Atom':
                            eng = main + ' ' + phrase
                            eng_list.append(eng)

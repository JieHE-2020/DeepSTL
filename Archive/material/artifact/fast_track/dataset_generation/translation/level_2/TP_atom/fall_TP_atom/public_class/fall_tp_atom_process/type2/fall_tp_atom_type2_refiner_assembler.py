...
class FallTPAtomType2RefinerAssembler:
        ...
        for i in range(len(predicate_refined)):
            for predicate in predicate_refined[i]:
                for appendix in appendix_refined:
                    # detect whether there is repetition between predicate and appendix
                    repetition_result = self.detect_repetition(predicate, appendix)
                    if repetition_result:
                        # if repetition is detected, adopt random dropping with a large probability
                    ...
                    if len(self.adverbial_para) != 0:  # temporarily set like this
                        ...
                        if flag == 0:
                            ...
                            # if repetition is detected but dropping has not been exerted, the translations added by
    ...
    @staticmethod
    def detect_repetition(predicate, appendix):
        result = False

        if ('detected' in predicate and 'detected' in appendix) or \
                ('observed' in predicate and 'observed' in appendix):
            result = True

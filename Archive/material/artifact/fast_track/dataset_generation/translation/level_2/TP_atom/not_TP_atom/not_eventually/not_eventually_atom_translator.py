from translation.level_2.TP_atom.not_TP_atom.not_eventually.direct_negation.\
    direct_negate_eventually_atom_translator import DirectNegateEventuallyAtomTranslator
from translation.level_2.TP_atom.not_TP_atom.not_eventually.appositive_appendix.\
    appositive_appendix_negate_eventually_atom_handler import AppositiveAppendixNegateEventuallyAtomHandler
from translation.level_2.TP_atom.original_TP_atom.always.normal.\
    always_atom_translator import AlwaysAtomTranslator
import copy
import math
import random


class NotEventuallyAtomTranslator:
    ...
    def direct_negate_translate_process(self, direct_negate_limit_num):
        direct_negate_translator = \
            DirectNegateEventuallyAtomTranslator(self.translate_guide, direct_negate_limit_num)
    ...
    def appositive_appendix_translate_process(self, appositive_appendix_limit_num):
        appositive_appendix_handler = \
            AppositiveAppendixNegateEventuallyAtomHandler(self.translate_guide, appositive_appendix_limit_num)
        appositive_appendix_translator = \
            appositive_appendix_handler.appositive_appendix_negate_eventually_atom_translator

        return appositive_appendix_translator

    def transform_translate_process(self, transform_limit_num):
        always_not_instruction = {
        ...
        guide = [always_not_instruction, self.translate_guide['always_not_info_dict']]
        whether_from_not_eventually = True
        # not (eventually expr) -> always (not expr)
        transform_translator = AlwaysAtomTranslator(guide, transform_limit_num, whether_from_not_eventually)

        return transform_translator

    def check_direct_negation_feasibility(self):
        feasibility = False
        not_eventually_info_dict = self.translate_guide['not_eventually_info_dict']
        sp_info_dict = not_eventually_info_dict['ingredient'][0]

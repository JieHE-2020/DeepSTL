from translation.level_2.TP_atom.not_TP_atom.not_historically.direct_negation.\
    direct_negate_historically_atom_handler import DirectNegateHistoricallyAtomHandler
from translation.level_2.TP_atom.not_TP_atom.not_historically.appositive_appendix.\
    appositive_appendix_negate_historically_atom_handler import AppositiveAppendixNegateHistoricallyAtomHandler
from translation.level_2.TP_atom.original_TP_atom.once.normal.\
    once_atom_translator import OnceAtomTranslator
import copy
import math
import random


class NotHistoricallyAtomTranslator:
    ...
    def direct_negate_translate_process(self, direct_negate_limit_num):
        direct_negate_handler = DirectNegateHistoricallyAtomHandler(self.translate_guide, direct_negate_limit_num)
    ...
    def appositive_appendix_translate_process(self, appositive_appendix_limit_num):
        appositive_appendix_handler = \
            AppositiveAppendixNegateHistoricallyAtomHandler(self.translate_guide, appositive_appendix_limit_num)
        appositive_appendix_translator = \
            appositive_appendix_handler.appositive_appendix_negate_historically_atom_translator

        return appositive_appendix_translator

    def transform_translate_process(self, transform_limit_num):
        once_not_instruction = {
        ...
        guide = [once_not_instruction, self.translate_guide['once_not_info_dict']]
        # not (historically expr) -> once (not expr)
        transform_translator = OnceAtomTranslator(guide, transform_limit_num)

        return transform_translator

    def check_direct_negation_feasibility(self):
        feasibility = False
        not_historically_info_dict = self.translate_guide['not_historically_info_dict']
        sp_info_dict = not_historically_info_dict['ingredient'][0]

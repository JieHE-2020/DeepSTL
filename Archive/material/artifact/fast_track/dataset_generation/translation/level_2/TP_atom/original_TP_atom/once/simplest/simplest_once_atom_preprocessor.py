...
from translation.level_2.TP_atom.original_TP_atom.once.simplest.SP.simplest_once_atom_template_refiner \
    import SimplestOnceAtomTemplateRefiner
from translation.level_2.TP_atom.original_TP_atom.once.normal.SP.once_atom_assembler import OnceAtomAssembler
import copy
import random


class SimplestOnceAtomPreprocessor:
    """
    functions:
    ...
    def main_sentence_process(self):
        main_sentence_refiner = SimplestOnceAtomTemplateRefiner(self.sp_info_dict, self.predicate_cmd_dict)
    ...
    def pack_key_list(self):
        return [self.eng_temporal_phrase, self.eng_main_sentence]
